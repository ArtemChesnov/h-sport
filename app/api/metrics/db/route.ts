/**
 * GET /api/metrics/db
 * Метрики БД: slow queries, connection pool. Только для администраторов.
 */

import { withErrorHandling } from "@/shared/lib/api/error-handler";
import { requireAdmin } from "@/shared/lib/auth/middleware";
import { withMetricsAuto } from "@/shared/lib/api/metrics-middleware";
import { getDbMetrics } from "@/shared/services/server/metrics/metrics-route.service";
...
export const GET = withMetricsAuto(async (request: NextRequest): Promise<NextResponse<unknown>> => {
  return withErrorHandling(
    async (req) => {
      ...
      const result = await getDbMetrics(periodHours);
