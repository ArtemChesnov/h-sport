...
const postHandlerWithMetrics = withMetricsAuto(async (
  request: NextRequest,
  ...
  for (const table of tablesToClean) {
    try {
      ...
      if (table.name === "ProductView" || table.name === "CartAction" ||
          table.name === "FavoriteAction" || table.name === "Conversion" ||
          table.name === "all") {
      ...
      if (table.name === "WebVitalsMetric" || table.name === "all") {
        try {
          const { cleanOldWebVitals } = await import(
            "@/shared/services/server/metrics/metrics-route.service"
          );
